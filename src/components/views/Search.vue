<template>
  <main id="Search-page">
    <form @submit.prevent="searchMovies">
      <input v-model="title" type="text" placeholder="제목을 입력하세요" class="search--input" />
    </form>
    <section>
      <div v-if="searchResults.length" class="Search--wrap">
        <swiper :effect="'cards'" :grabCursor="true" :modules="modules" class="mySwiper--search">
          <swiper-slide v-for="movie in searchResults" :key="movie.id" class="swiper-slide--search">
            <div :class="['card', { 'card--flipped': selectedMovie === movie }]">
              <div class="card__front" @click="flipCard(movie)">
                <img :src="getMoviePosterUrl(movie.poster_path)" alt="Movie Poster" class="movieImg--search" />
              </div>
